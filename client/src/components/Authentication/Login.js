    ...
    const onSubmit = data => {
        if (returningUser) {
            if (data.email && data.password) {
                auth.signIn(data.email, data.password,lerr,setlErr);   
            }
           
        } else {
            if (data.name && data.email && data.password && data.confirm_password) {
                // console.log(data.email, data.confirm_password, data.name);
                auth.signUp(data.email, data.confirm_password, data.name,serr,setsErr)
                ...
                </div>
                {
                    returningUser ?
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="py-3">
                            ...
                            {
                                lerr.length!=0 && <p className="text-danger"> User Not Found</p>
                            }
                        ...
                        {
                          serr.length!=0 && <p className="text-danger">Email Already Taken</p>
